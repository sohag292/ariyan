...
const loadToolsData = async (dataLimit, sortDateAscending) => {
  toggleSpinner(true);
...
const displayTools = (tools, dataLimit) => {
  ...
  let displayedTools = dataLimit ? tools.slice(0, dataLimit) : tools;
  ...
  const showAllButton = document.getElementById("seeMoreAll");
  showAllButton.classList[dataLimit && dataLimit < tools.length ? "remove" : "add"]("d-none");
...
//tooglerSpinner
const toggleSpinner = isLoading => document.getElementById('loaderSpinner').classList.toggle('d-none', !isLoading);
...
document.getElementById("btn-seeMoreAll").addEventListener("click", function () {
  processSearch();
});
...
const displayDetails = Ai => {
  ...
  AiContainer.innerHTML = ''; // clear previous modal data
  ...
  AiDiv.innerHTML = `
      ...
      <div class="border border-danger rounded bg-light bg-gradient p-3">
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <ul>
              <li>${Ai.features ? Ai.features["1"].feature_name : ' Data Not Found'}</li>
              <li>${Ai.features ? Ai.features["2"].feature_name : ' Data Not Found'}</li>
              <li>${Ai.features ? Ai.features["3"].feature_name : ' Data Not Found'}</li>
            </ul>
          </div>
          <div class="col-md-6">
    ...
    <div class="position-relative">
      ...
      <div class="text-center mt-4">
        <h4>${Ai.input_output_examples ? Ai.input_output_examples[0].input : "Can you give any example?"}</h4>
